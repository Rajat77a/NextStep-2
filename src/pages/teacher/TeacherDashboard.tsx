...
export default function TeacherDashboard() {
  const { user } = useAuth();
  const [stats, setStats] = useState({ totalStudents: 0, reportCardCount: 0, flaggedCount: 0 });
  ...
  useEffect(() => {
    async function load() {
      if (!user) return;
      const s = await getTeacherDashboard();
      setStats(s);
  ...
  const openStudentPanel = async (student: Student) => {
    ...
    if (cards.length > 0) {
      const latest = cards.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
      setStudentGrades(storage.getSubjectGrades().filter(g => g.reportCardId === latest.id));
    }
  ...
  const handleAddNote = async () => {
    ...
    await addTeacherNote({ studentId: selectedStudent.id, note: noteText });
    const notes = await getTeacherNotes(selectedStudent.id);
    setStudentNotes(notes);
    setNoteText('');
  };

  const watchList = [...students].sort((a, b) => {
    const fa = getStudentFlag(a.id) === 'red' ? 2 : getStudentFlag(a.id) === 'yellow' ? 1 : 0;
    const fb = getStudentFlag(b.id) === 'red' ? 2 : getStudentFlag(b.id) === 'yellow' ? 1 : 0;
    return fb - fa;
  }).slice(0, 5);
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 md:px-12 py-6 md:py-8">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        <h2 className="font-display text-2xl md:text-4xl text-charcoal mb-1">My Classes</h2>
        <p className="font-body text-medium-gray mb-6">Overview of your students and their progress</p>
      </motion.div>
      ...
      <div className="grid lg:grid-cols-[2fr_1fr] gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl shadow-card p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
            ...
            <div className="relative w-full sm:w-56">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search students..." className="w-full pl-9 pr-4 py-2 rounded-lg border border-light-gray bg-cream font-body text-sm focus:border-coral outline-none" />
            </div>
          </div>
                ...
                {filteredStudents.map(student => {
                  ...
                  return (
                    <tr
                      ...
                      className={`border-b border-light-gray/50 cursor-pointer hover:bg-cream/50 transition-colors ${flag === 'red' ? 'bg-coral/[0.02]' : ''}`}
                    >
                      ...
                      <td className="py-3 px-4"><span className={`w-2 h-2 rounded-full inline-block ${flag === 'red' ? 'bg-coral' : flag === 'yellow' ? 'bg-amber' : 'bg-sage'}`} /></td>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-2xl shadow-card p-6">
          <h3 className="font-display text-xl text-charcoal mb-4">Students to Watch</h3>
          <div className="space-y-3">
            {watchList.map(student => {
              ...
              return (
                ...
                >
                  <div>
                    <p className="font-body font-medium text-sm text-charcoal">{student.fullName}</p>
                    <p className="font-body text-xs text-medium-gray">Roll {student.rollNumber}</p>
                  </div>
          ...
          <Link to="/teacher/patterns" className="flex items-center gap-1 text-coral font-body text-sm font-semibold mt-4 hover:underline">
      ...
      {showPanel && selectedStudent && (
        ...
        >
          <div className="absolute inset-0 bg-charcoal/30 backdrop-blur-sm" onClick={() => setShowPanel(false)} />
          <motion.div
            initial={{ x: 400 }}
            animate={{ x: 0 }}
          ...
          >
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="font-display text-xl text-charcoal">{selectedStudent.fullName}</h3>
                  <p className="font-body text-sm text-medium-gray">Roll {selectedStudent.rollNumber}</p>
                </div>
                <button onClick={() => setShowPanel(false)} className="w-8 h-8 rounded-full bg-cream flex items-center justify-center hover:bg-light-gray transition-colors">
                  ✕
                </button>
              </div>
              ...
              <div className="space-y-3 mb-6">
                {studentGrades.map(g => (
                  <div key={g.id} className="flex items-center justify-between p-3 bg-cream rounded-xl">
                    <span className="font-body text-sm text-charcoal">{g.subjectName}</span>
                    <div className="flex items-center gap-2">
                      <span className="font-body text-sm text-medium-gray">{g.grade}</span>
              ...
              {/* Teacher Notes */}
              <div className="mb-4">
                <h4 className="font-body font-semibold text-sm text-charcoal mb-3">My Notes</h4>
                {studentNotes.map(n => (
                  <div key={n.id} className="bg-cream rounded-lg p-3 mb-2">
                    <p className="font-body text-sm text-charcoal">{n.note}</p>
                  </div>
                ))}
                <div className="flex gap-2">
                  ...
                  />
                  <button onClick={handleAddNote} className="px-4 py-2 rounded-lg bg-coral text-white font-body text-sm hover:bg-coral-dark transition-all">Add</button>
                </div>
              </div>

              <div className="bg-amber/[0.06] rounded-lg p-3">
                <p className="font-body text-xs text-medium-gray italic">Parent insights are private. You can see class-level patterns only.</p>
              </div>
            </div>
          </motion.div>
        </motion.div>
